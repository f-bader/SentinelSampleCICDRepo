...
Describe "Sentinel Configuration" -Tag "Configuration", "Sentinel" {
    ...
    It "Microsoft data access is disabled (EyesOn)" -Tag "EyesOn" {
        $SentinelSettings | Where-Object { $_.name -eq "EyesOn" } | Select-Object -ExpandProperty properties | Select-Object -ExpandProperty isEnabled | Should -BeNullOrEmpty
    }
